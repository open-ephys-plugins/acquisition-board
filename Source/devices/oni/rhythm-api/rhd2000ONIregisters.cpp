//----------------------------------------------------------------------------------
// Rhd2000ONIRegisters.cpp
//
// Intan Technoloies RHD2000 Rhythm Interface API
// Rhd2000ONIRegisters Class
...
Rhd2000ONIRegisters::Rhd2000ONIRegisters (double sampleRate)
{
    aPwr.resize (64);
...
void Rhd2000ONIRegisters::defineSampleRate (double newSampleRate)
{
...
// if enabled.
void Rhd2000ONIRegisters::setFastSettle (bool enabled)
{
    ampFastSettle = (enabled ? 1 : 0);
}

// Drive auxiliary digital output low
void Rhd2000ONIRegisters::setDigOutLow()
{
    digOut = 0;
    digOutHiZ = 0;
}
...
void Rhd2000ONIRegisters::setDigOutHigh()
{
    digOut = 1;
    digOutHiZ = 0;
}
...
void Rhd2000ONIRegisters::setDigOutHiZ()
{
    digOut = 0;
    digOutHiZ = 1;
}
...
void Rhd2000ONIRegisters::enableAux1 (bool enabled)
{
    adcAux1En = (enabled ? 1 : 0);
}

// Enable or disable ADC auxiliary input 2
void Rhd2000ONIRegisters::enableAux2 (bool enabled)
{
    adcAux2En = (enabled ? 1 : 0);
}

// Enable or disable ADC auxiliary input 3
void Rhd2000ONIRegisters::enableAux3 (bool enabled)
{
    adcAux3En = (enabled ? 1 : 0);
}

// Enable or disable DSP offset removal filter
void Rhd2000ONIRegisters::enableDsp (bool enabled)
{
    dspEn = (enabled ? 1 : 0);
}
...
double Rhd2000ONIRegisters::setDspCutoffFreq (double newDspCutoffFreq)
{
    int n;
...
double Rhd2000ONIRegisters::getDspCutoffFreq() const
{
    double x;
    const double Pi = 2 * acos (0.0);
...
void Rhd2000ONIRegisters::enableZcheck (bool enabled)
{
    zcheckEn = (enabled ? 1 : 0);
}

// Power up or down impedance checking DAC
void Rhd2000ONIRegisters::setZcheckDacPower (bool enabled)
{
...
void Rhd2000ONIRegisters::setZcheckScale (ZcheckCs scale)
{
    switch (scale)
    {
        case ZcheckCs100fF:
...
void Rhd2000ONIRegisters::setZcheckPolarity (ZcheckPolarity polarity)
{
    switch (polarity)
    {
...
int Rhd2000ONIRegisters::setZcheckChannel (int channel)
{
...
void Rhd2000ONIRegisters::setAmpPowered (int channel, bool powered)
{
    if (channel >= 0 && channel <= 63)
    {
...
void Rhd2000ONIRegisters::powerUpAllAmps()
{
...
void Rhd2000ONIRegisters::powerDownAllAmps()
{
    for (int channel = 0; channel < 64; ++channel)
    {
        aPwr[channel] = 0;
...
int Rhd2000ONIRegisters::getRegisterValue (int reg) const
{
    int regout;
    ...
    {
        case 0:
            regout = (adcReferenceBw << 6) + (ampFastSettle << 5) + (ampVrefEnable << 4) + (adcComparatorBias << 2) + adcComparatorSelect;
            break;
        case 1:
        ...
        case 2:
            ...
            break;
        case 3:
            regout = (muxLoad << 5) + (tempS2 << 4) + (tempS1 << 3) + (tempEn << 2) + (digOutHiZ << 1) + digOut;
            break;
        case 4:
        ...
        case 13:
            ...
            break;
        case 14:
            regout = (aPwr[7] << 7) + (aPwr[6] << 6) + (aPwr[5] << 5) + (aPwr[4] << 4) + (aPwr[3] << 3) + (aPwr[2] << 2) + (aPwr[1] << 1) + aPwr[0];
            break;
        case 15:
            regout = (aPwr[15] << 7) + (aPwr[14] << 6) + (aPwr[13] << 5) + (aPwr[12] << 4) + (aPwr[11] << 3) + (aPwr[10] << 2) + (aPwr[9] << 1) + aPwr[0];
            break;
        case 16:
            regout = (aPwr[23] << 7) + (aPwr[22] << 6) + (aPwr[21] << 5) + (aPwr[20] << 4) + (aPwr[19] << 3) + (aPwr[18] << 2) + (aPwr[17] << 1) + aPwr[16];
            break;
        case 17:
            regout = (aPwr[31] << 7) + (aPwr[30] << 6) + (aPwr[29] << 5) + (aPwr[28] << 4) + (aPwr[27] << 3) + (aPwr[26] << 2) + (aPwr[25] << 1) + aPwr[24];
            break;
        case 18:
            regout = (aPwr[39] << 7) + (aPwr[38] << 6) + (aPwr[37] << 5) + (aPwr[36] << 4) + (aPwr[35] << 3) + (aPwr[34] << 2) + (aPwr[33] << 1) + aPwr[32];
            break;
        case 19:
            regout = (aPwr[47] << 7) + (aPwr[46] << 6) + (aPwr[45] << 5) + (aPwr[44] << 4) + (aPwr[43] << 3) + (aPwr[42] << 2) + (aPwr[41] << 1) + aPwr[40];
            break;
        case 20:
            regout = (aPwr[55] << 7) + (aPwr[54] << 6) + (aPwr[53] << 5) + (aPwr[52] << 4) + (aPwr[51] << 3) + (aPwr[50] << 2) + (aPwr[49] << 1) + aPwr[48];
            break;
        case 21:
            regout = (aPwr[63] << 7) + (aPwr[62] << 6) + (aPwr[61] << 5) + (aPwr[60] << 4) + (aPwr[59] << 3) + (aPwr[58] << 2) + (aPwr[57] << 1) + aPwr[56];
            break;
        default:
            regout = -1;
    }
...
double Rhd2000ONIRegisters::rH1FromUpperBandwidth (double upperBandwidth) const
{
...
// bandwidth value (in Hz).
double Rhd2000ONIRegisters::rH2FromUpperBandwidth (double upperBandwidth) const
{
    double log10f = log10 (upperBandwidth);
...
// bandwidth value (in Hz).
double Rhd2000ONIRegisters::rLFromLowerBandwidth (double lowerBandwidth) const
{
...
double Rhd2000ONIRegisters::upperBandwidthFromRH1 (double rH1) const
{
    double a, b, c;
...
double Rhd2000ONIRegisters::upperBandwidthFromRH2 (double rH2) const
{
    double a, b, c;
...
double Rhd2000ONIRegisters::lowerBandwidthFromRL (double rL) const
{
    double a, b, c;
...
double Rhd2000ONIRegisters::setUpperBandwidth (double upperBandwidth)
{
    ...
    /*
    cout << endl;
    cout << "Rhd2000ONIRegisters::setUpperBandwidth" << endl;
...
double Rhd2000ONIRegisters::setLowerBandwidth (double lowerBandwidth)
{
    ...
    cout << endl;
    cout << fixed << setprecision(1);
    cout << "Rhd2000ONIRegisters::setLowerBandwidth" << endl;
...
int Rhd2000ONIRegisters::createRhd2000Command (Rhd2000CommandType commandType)
{
    switch (commandType)
    {
        ...
        case Rhd2000CommandCalClear:
            ...
            break;
        default:
            cerr << "Error in Rhd2000ONIRegisters::createRhd2000Command: "
...
int Rhd2000ONIRegisters::createRhd2000Command (Rhd2000CommandType commandType, int arg1)
{
    switch (commandType)
    {
...
int Rhd2000ONIRegisters::createRhd2000Command (Rhd2000CommandType commandType, int arg1, int arg2)
{
    switch (commandType)
    {
...
int Rhd2000ONIRegisters::createCommandListRegisterConfig (vector<int>& commandList, bool calibrate)
{
...
//
// Returns the length of the command list.
int Rhd2000ONIRegisters::createCommandListTempSensor (vector<int>& commandList)
{
    int i;
...
//
// Returns the length of the command list.
int Rhd2000ONIRegisters::createCommandListUpdateDigOut (vector<int>& commandList)
{
    int i;
...
// Returns the length of the command list.
int Rhd2000ONIRegisters::createCommandListZcheckDac (vector<int>& commandList, double frequency, double amplitude)
{
    int i, period, value;
    double t;
    const double Pi = 2 * acos (0.0);
    ...
    {
        cerr << "Error in Rhd2000ONIRegisters::createCommandListZcheckDac: Amplitude out of range." << endl;
        return -1;
    }
    if (frequency < 0.0)
    {
        cerr << "Error in Rhd2000ONIRegisters::createCommandListZcheckDac: Negative frequency not allowed." << endl;
        return -1;
    }
    else if (frequency > sampleRate / 4.0)
    {
        cerr << "Error in Rhd2000ONIRegisters::createCommandListZcheckDac: "
    ...
    else
    {
        ...
        {
            cerr << "Error in Rhd2000ONIRegisters::createCommandListZcheckDac: Frequency too low." << endl;
            return -1;
        }
        else
        {
